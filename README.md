...
### Chromium 系でキャレットが残る件（対策済み）

Chromium は、フォーカスがテキスト以外の要素（ツールバーのボタンなど）へ移っても、MSAA 向けに用意しているシステムキャレットを破棄しない。座標を保持したまま、幅だけ 0 になった矩形を返す。

この幅 0 の矩形を「テキスト欄にフォーカスが無い」信号として扱い、キャレット無しと判定する。ここで UI Automation に問い合わせてはならない。Web ページ本体が `TextPattern` に応答するため、別位置にバッジが出る不具合になる。
