...
| Visual Studio Code の検索欄（`Ctrl+F` / `Ctrl+Shift+F`） | 打ち始めると消える | キャレット矩形が打鍵に追従して1文字あたり13px動くため、移動判定が最初の1打鍵で発火する |
...
Visual Studio Code の `Ctrl+F` と `Ctrl+Shift+F` の検索欄では、打ち始めるとバッジが消える。100ms 間隔で計測したところ、UIA が返すキャレット矩形は `1x17` で、打鍵にあわせて x 座標が `1020 → 1033 → 1046 → 1059 → 1072` と1文字あたり13px動いていた。**「2px 以上動いたら入力開始とみなす」規則が最初の1打鍵で発火している。** 仕様どおりの動作であり、Windows Terminal のようにアプリ側が勝手に矩形を動かしているのではない。

**未解決**: 同じ計測で、エディタ本体にフォーカスがあるあいだ UIA が返したのは `462x19`（比 24.3）と `731x19`（比 38.5）で、いずれも妥当性ガードに掛かる。MSAA は 202 サンプルすべて全ゼロだった。この結果だとエディタではバッジが出せないはずだが、実際には出る。外部プロセスからの計測が、アプリ自身が見ているものと食い違っている。原因は特定できていない。
