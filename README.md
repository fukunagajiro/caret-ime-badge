...
- 文字を打ち始める（キャレットが動く）と、その時点でフェードアウトが始まる — 入力前の確認が目的なので、入力中はいつまでも居座らない（ただし空の入力欄で位置を組み立てた場合はこの限りではなく、位置は固定される。詳細は下記「既知の制約」参照）
...
終了コードが失敗件数（0 なら全件成功）。現在 124 件のアサーションがある。
