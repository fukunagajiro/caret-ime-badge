...
- **XAML/WinUI 系の入力欄（メモ帳の `Ctrl+F` 検索ボックスなど。エクスプローラーの検索欄は空でなくてもそもそも動作しない — 上記参照）では、空の状態でバッジが出ないか、出てもごく一瞬で消える。対応しない。** 理由は二つある。第一に、UIA はこれらの入力欄が空のときキャレットの矩形を返せない（MSAA も全ゼロを返す）ため、フォーカスした瞬間には表示すべき位置そのものが無い。第二に、要素自身の矩形で代用しようとしても、その矩形はレイアウトの再計算のたびに動き、ユーザーの入力によるキャレット移動と区別がつかない。実測ではメモ帳の検索バーの要素矩形が 120ms のポーリング間隔のあいだに最大 95px 動いており、これに「2px 以上動いたら入力開始とみなす」規則を適用するとバッジは表示直後に消える。表示直後の一定時間だけこの移動判定を止める猶予期間を実装して緩和を試みたが、Windows Terminal の新規タブでバッジの実効表示時間が 800ms の設定に対し 120〜380ms に短縮される事例が残り、実使用でも「プロンプトが表示される前にバッジの表示が終わる」という体感が拭えなかったため、取り消した。
